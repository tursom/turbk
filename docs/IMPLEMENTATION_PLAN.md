...
- Credentials：SFTP、FTP、WebDAV 凭据管理。
...
1. Agent 使用 token 创建 backup run。
2. Agent 扫描本地文件树。
...
- 凭据、Agent token、连接密钥在数据库中加密存储。
- Web 用户使用登录会话。
- Agent 使用独立 token，不复用 Web 用户会话。
...
- Agent token 作为 `agent` 类型凭据加密保存，Agent API 必须使用 `Authorization: Bearer <token>`。
- Agent 创建 run 时按 token 凭据和 job name 自动创建或复用 `agent` job；同一 job 已有 pending/running run 时返回原 run。

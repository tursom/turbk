...
本文定义 Turbk Agent 的 7x24 常驻模式、本地 catalog、服务端异步命令和 chunk generation 同步机制。该设计基于当前已有的 Agent HTTP API、Client ID / Secret 认证、服务端 canonicalize manifest、append-only segment 仓库和维护任务。
...
- Agent 使用本地 catalog 保存文件元数据、文件 chunk 列表和服务端 chunk 确认状态；首版可以是 SQLite，写入热点后续迁到 Pebble hybrid catalog。
- 对本地 catalog 已确认存在于服务端的 chunk，不再逐个请求服务端。
- 对本地 catalog 未确认的 chunk，新 Agent 必须使用批量 check/upload，公网主路径不逐 chunk GET/PUT。
...
- 单 chunk GET/PUT 只作为旧 agent 兼容接口；新 agent 的扫描、上传和缺失修复路径必须走批量接口。
...
1. 启动后打开本地 catalog，默认 `hybrid` backend。
...
- 状态目录互斥：使用 `agent.lock` 防止多个 Agent 进程共享同一 `state_dir` 时并发运行。
...
当前默认 catalog backend 是 `hybrid`：SQLite 保存低频状态，Pebble 保存高频文件记录和 chunk 确认状态。需要回滚时可设置 `TURBK_AGENT_CATALOG_BACKEND=sqlite` 使用旧 SQLite-only 行为。
...
`files`（`sqlite` backend 使用；`hybrid` backend 对应 Pebble `0x02` record）：
...
`file_chunks`（`sqlite` backend 使用；`hybrid` backend 合并进 Pebble file record value）：
...
`server_chunks`（`sqlite` backend 使用；`hybrid` backend 对应 Pebble `0x01` record）：
...
扫描文件时，Agent 先读取文件元数据。若 path、type、size、mode、uid、gid、mtime、dev、inode 与本地 file catalog 一致，则认为文件未变化：

- 不重新读取文件内容。
- 直接复用本地 file catalog 中保存的 chunk hash 列表。
- 对这些 chunk 只在 generation 需要时做批量校验。

如果元数据变化，则重新读取文件并分块。分块后更新本地 file catalog。
...
3. 如果 `repository.id` 和 generation 均未变化，Agent 可以信任本地 chunk catalog 中 `confirmed` 的记录。
4. 未变化文件直接复用本地 file catalog 中保存的 chunk 列表。
...
1. `lstat` 读取元数据。
2. 检查伪文件系统和排除规则。
3. 如果本地 catalog 判断未变化，复用本地 file catalog 中保存的 chunk 列表。
4. 如果变化，读取文件并使用内容定义分块。
5. 对本次扫描产生的 chunks 分批处理：
   - 本地 chunk catalog 在当前 `chunk_generation` 已 confirmed：直接引用。
   - 本地没有记录或 generation 过旧：加入批量 check。
   - check 缺失或未知：加入批量 upload。
6. 更新进度。
...
2. 重新读取文件并重新分块。
3. 如果文件元数据和扫描时一致，使用批量 upload 补传缺失 chunk。

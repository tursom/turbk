...
5. Agent 只能创建或更新该 host 范围内的 jobs/runs/snapshots。
6. Heartbeat 更新 host.status、host.address、host.last_seen_at。
...
后端根据 host.source_type 填充或校验 job.source_type，根据 host.credential_id 执行连接。

`PATCH /api/v1/jobs/:id`

允许修改任务名称、source_config、enabled、schedule、timezone、max_runtime_seconds、retry_attempts。默认不提供 credential_id 修改入口。

是否允许修改 job.host_id 可以后续决定；首版建议不在普通编辑表单中提供，避免任务历史语义混乱。
...
3. 页面展示 Client ID / Secret、Server URL、Docker/Compose 配置。
4. Secret 可以重复显示。
...
- 创建 local host 时，credential_id 必须为空。
- 创建 Agent host 时，不允许客户端提交 client_id/client_secret；服务端生成。
- Agent client_id 必须通过唯一索引查找，认证路径不得扫描或解密所有 credentials。
- Agent secret 校验必须使用 hash + constant-time compare。
- 创建 job 时，host_id 必填，host 必须存在。
...
6. 调整 Agent API：认证后解析绑定 host，并确保 run/job 属于该 host。
...
- Agent credential 在 Credentials 页面管理。
- 按用户或租户隔离 credential。
- 客户端零信任加密。
